<template>
  <div class="flex flex-col w-full border-l border-gray-500 overflow-x-auto">
    <div class="flex shrink-0 h-12 border-b border-gray-500">
      ...
      <div class="flex h-full ml-auto pr-4 items-center gap-4 snug">
        <div class="flex flex-col items-start justify-center h-full">
          ...
          <select v-model="displayOptions.barWidth"
        ...
        <div v-if="tableMode !== 'chance'" class="flex flex-col items-start justify-center h-full">
          <div class="text-sm">Display:</div>
          <select v-model="(displayOptions as DisplayOptionsBasics).content"
            class="w-28 px-1 py-0.5 border-gray-600 bg-gray-200 rounded-lg shadow cursor-pointer bg-right"
            @change="updateDisplayOptions">
        ...
        <Tippy content="Export summary to CSV file">
          <button :class="'flex w-8 h-8 items-center justify-center border border-gray-600 bg-gray-200 ' +
        ...
        <Tippy content="Export weights to TXT file">
          <button :class="'flex w-8 h-8 items-center justify-center border border-gray-600 bg-gray-200 ' +
            'rounded-lg shadow transition-colors active:bg-gray-300'
            " @click="exportWeights">
    ...
    <div ref="tableDiv" class="flex-grow overflow-y-scroll will-change-scroll" @scroll.passive="onTableScroll">
      <table class="w-full h-full text-sm text-center align-middle">
        <thead class="sticky top-0 z-30 bg-gray-100 shadow">
          <tr style="height: calc(1.9rem + 1px)">
            <th v-for="column in columns" :key="column.label" scope="col" :class="'whitespace-nowrap select-none ' +
              (column.type === 'card'
              ...
              <span v-if="sortKey.key === columnIndex(column)" class="inline-block text-xs pr-1">
                {{ sortKey.order === "asc" ? "▲" : "▼" }}
              </span>
          ...
          <tr v-if="tableMode !== 'chance'" style="height: calc(1.9rem + 1px)">
            <th v-for="column in columns" :key="column.label" scope="col" :class="'header-divider ' +
              (column.type === 'card'
              ...
              <template v-else-if="column.type === 'bar'">
                <div v-if="summary && column.label === 'Strategy'"
                  class="w-full h-full bg-neutral-800 bg-left bg-no-repeat" :style="{
              ...
              <template v-else>
                <div class="inline-block w-12 text-right">
                  <span v-if="column.type === 'weight'" :data-set="(strTmp = toFixedAdaptive(summary[columnIndex(column)]))
                    ">
                  ...
                  " :data-set="(strTmp = toFixed1(summary[columnIndex(column)] * 100))
                    ">
                    <span>{{ strTmp.slice(0, -1) }}</span>
                    <span class="text-xs">{{ strTmp.slice(-1) }}%</span>
                  </span>
                  <span v-else-if="column.type === 'action-ev'" :data-set="(strTmp = toFixed1(
                  ...
                  <span v-else-if="column.type === 'ev'" :data-set="(strTmp = toFixed[evDigits - 1](
              ...
              " class="absolute w-12 h-1 bottom-0 inset-x-0 mx-auto" :style="{
        ...
        <tbody>
          <!-- Top empty row -->
          <tr v-if="emptyBufferTop > 0" :style="{
          ...
          <!-- Body -->
          <tr v-for="item in resultsRendered" :key="item[0]" :class="'relative ' +
            (item[0] === scrollTarget ? 'bg-yellow-200' : 'bg-gray-50')
            " style="height: calc(1.9rem + 1px)">
            <td v-for="column in columns" :key="column.label" :class="'row-divider ' +
              (column.type === 'card'
                ? 'sticky left-0 z-10 ' +
                (item[0] === scrollTarget
                  ? 'bg-yellow-200 '
              ...
              <template v-if="column.type === 'card'">
                <span v-for="card in pairText(item[columnIndex(column)])" :key="card.rank + card.suit"
              ...
              <template v-else-if="column.type === 'bar'">
                <div class="w-full h-full bg-neutral-800 bg-left bg-no-repeat" :style="{
              ...
              <template v-else>
                <div class="inline-block w-12 text-right">
                  <span v-if="tableMode === 'chance' && column.type === 'weight'" :data-set="(strTmp = toFixedAdaptive(item[columnIndex(column)]))
                    ">
                    <span>{{ strTmp.slice(0, strTmp.indexOf(".") + 1) }}</span>
                    <span class="text-xs">{{
                      strTmp.slice(strTmp.indexOf(".") + 1)
                  ...
                  <span v-else-if="
                    ...
                    column.type === 'action'
                  " :data-set="(strTmp = toFixed1(item[columnIndex(column)] * 100))
                    ">
                    <span>{{ strTmp.slice(0, -1) }}</span>
                    <span class="text-xs">{{ strTmp.slice(-1) }}%</span>
                  </span>
                  ...
                  " :data-set="(strTmp = toFixed[evDigits - 1](
              ...
              <div v-if="column.type === 'action' || column.type === 'action-ev'"
                class="absolute w-12 h-1 bottom-0 inset-x-0 mx-auto" :style="{
          ...
          <tr v-if="resultsRendered.length === 0">
            <td class="relative bg-gray-50 row-divider" style="height: calc(1.9rem + 1px)" :colspan="columns.length">
              {{
                tableMode === "chance"
          ...
          <!-- Bottom empty row -->
          <tr v-if="emptyBufferBottom > 0" :style="{
...
import { ArrowTopRightOnSquareIcon, DocumentTextIcon } from "@heroicons/vue/24/solid";
...
const actionBarBg = (index: number, row: number[]) => {
  ...
  return `linear-gradient(to right, ${color} 0% ${value * 100
...
const exportWeights = async () => {
  ...
  const playerIndex = props.displayPlayer === "oop" ? 0 : 1;
  const playerName = props.displayPlayer.toUpperCase();
  
  const filePath = await save({
    defaultPath: `weights_${playerName}.txt`,
    filters: [{ name: "Text Files", extensions: ["txt"] }],
  });
  
  if (filePath) {
    try {
      await invokes.exportWeights(playerIndex, filePath);
    } catch (error) {

<template>
  <div class="w-full h-full">
    <table class="w-full h-full table-fixed select-none snug" :style="{
      ...
      <tr v-for="row in 13" :key="row">
        <td v-for="col in 13" :key="col" :class="'relative border border-black ' +
          ...
          " @click="onClickCell(row, col)" @mouseenter="onMouseEnterCell(row, col)">
          <div :class="'flex absolute w-full h-full left-0 top-0 ' +
            (row === col ? 'bg-neutral-700' : 'bg-neutral-800')
            ">
            <div v-for="(column, k) in columns(row, col)" :key="k" class="flex-grow h-full bg-left-bottom bg-no-repeat"
              :style="{
          ...
          <div class="absolute bottom-px right-1 z-10 text-shadow text-white overflow-hidden" style="
              max-width: calc(100% - 0.25rem);
              font-size: var(--value-font-size);
            " :data-set="(strTmp = cellValueText[cellIndex(row, col)])">
            {{ strTmp.split(".")[0]
            }}<span v-if="strTmp.includes('.')">.<span style="font-size: 85%">{{
              strTmp.split(".")[1]
                }}</span></span>
          </div>
        </td>

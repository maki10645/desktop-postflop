...
import type { Results, Spot, SpotChance } from "../result-types";
...
const Adaptive = (props: { value: number; class: object }) => {
	...
	if (Number.isNaN(props.value)) return h("div", { class: "px-1" }, "-");
...
const BarChart = (props: { values: number[] }) => {
	if (
		...
		Number.isNaN(props.values[0]) ||
		Number.isNaN(props.values[1])
	) {
...
const eqr = computed(() => {
	...
	if (!Number.isFinite(ret[0])) ret[0] = Number.NaN;
	if (!Number.isFinite(ret[1])) ret[1] = Number.NaN;
	return ret;
});
